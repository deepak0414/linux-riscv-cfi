...
int arch_get_shadow_stack_status(struct task_struct *t, unsigned long __user *status)
{
	...
	bcfi_status |= info->user_cfi_state.ubcfi_en? ((1UL << 1) | (info->user_cfi_state.user_shdw_stk)): 0;

	copy_to_user(status, &bcfi_status, sizeof(bcfi_status));
...
int arch_set_shadow_stack_status(struct task_struct *t, unsigned long __user *status)
{
	...
	copy_from_user(&bcfi_status, status, sizeof(bcfi_status));
...
int arch_get_indir_br_lp_status(struct task_struct *t, unsigned long __user *status)
{
	...
	copy_to_user(status, &fcfi_status, sizeof(fcfi_status));
...
int arch_set_indir_br_lp_status(struct task_struct *t, unsigned long __user *status)
{
	...
	copy_from_user(&fcfi_status, status, sizeof(fcfi_status));
